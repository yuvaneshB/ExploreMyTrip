...
export const ManagerProfile = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-xl md:text-3xl font-extrabold tracking-tight text-slate-800 font-sans">Manager Profile</h1>
      ...
      <div className="max-w-3xl bg-white border border-slate-200 rounded-3xl p-6 md:p-8 space-y-6 shadow-sm text-left">
        <div className="flex items-center gap-4">
          ...
          <div>
            <strong className="text-lg text-slate-800 block font-bold">{user?.name || 'Manager Account'}</strong>
        ...
        <div className="border-t border-slate-100 pt-6 grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs font-semibold">
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Manager ID</span>
            <strong className="text-slate-800 font-bold mt-0.5 block">M-4190-OPS</strong>
          </div>
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Assigned Role</span>
            <strong className="text-slate-800 font-bold mt-0.5 block">{user?.role || 'Manager'}</strong>
          </div>
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Department</span>
            <strong className="text-slate-800 font-bold mt-0.5 block">Operations & Moderation</strong>
          </div>
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Phone Number</span>
            <strong className="text-slate-800 font-bold mt-0.5 block">+1 (555) 102-3940</strong>
          </div>
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Last Login Session</span>
            <strong className="text-slate-800 font-bold mt-0.5 block">{new Date().toDateString()}</strong>
          </div>
        </div>
